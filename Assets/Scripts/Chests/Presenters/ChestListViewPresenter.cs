...
public class ChestListViewPresenter : IChestListViewPresenter
{
	...
	public ChestListViewPresenter(ChestsService chestsService)
	{
		chestsService.CurrentChests.ObserveAdd().Subscribe(x =>
		{
			var presenter = new ChestViewPresenter(x.Value, chestsService);
			_chestPresenters.Add(presenter);
			_chestPresenterConnections.Add(x.Value, presenter);
		}).AddTo(_disposable);

		chestsService.CurrentChests.ObserveRemove().Subscribe(x =>
		{
